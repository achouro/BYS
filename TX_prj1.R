#Interest over time Data from Google Trends
...
library(gtrendsR)
res= gtrends(keyword = "Texas", geo = "", time= "today+5-y", gprop=c("web")) 
res= gtrends()
plot(res)

dat=read_excel('/Users/mac/Downloads/multiTimeline (2).csv')
head(dat)

dat$interest=dat$Texas...Worldwide.
plot(dat$interest, xlab="Monthly Time Increments",ylab="Interest", main = "Google Search Interests for Texas 2004-2024" )
lines(dat$interest, col='violet',  type='l', lty=1)
datum=dat[,-2]
head(datum)
...
# NDLM model that assumes observational variance v unknown, 
#system covariance known/specified using discount factor
...
plot(data) 
n=length(data$yt) #n=100 observations 
k=6
T=n-k
data_T=data$yt[1:T]
test_data=data$yt[(T+1):n]
data=list(yt = data_T)
...
## plot results
index=seq(2004,2024)
index=seq.Date(from = as.Date('2004-01-01'), to = as.Date('2004-03-01'), by = "1 month")
index_filt=index[1:T]
index_forecast=index[(T+1):(T+k)]

plot(index, data$interest, main = "Google Search for Texas ",type='l',
     xlab="Time",ylab="Interest",lty=3,ylim=c(400,1500))
points(index,data%yt,pch=20)


plot(dat$interest, xlab="Monthly Time Increments",ylab="Interest", main = "Google Search for Texas" )
lines(dat$interest, col='violet',  type='l', lty=1)
