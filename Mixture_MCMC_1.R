...
install.packages("https://github.com/dkahle/invgamma")
install.packages("https://cran.r-project.org/package=invgamma")
...
#sample component 1 or 2 n times with probabilities w for 1 and (1-w) for 2 with replacement
#given the samples for the component cc (either 1 or 2)
#draw the observations from either a normal dist with parameters of cc[1] or cc[2]
x  = rep(0, n)
for(i in 1:n){
...
dd  = 2          #Inverse Gamma priors alpha
qq  = 1          #Inverse Gamma priors beta
...
# Storing the samples
cc.out    = array(0, dim=c(rrr, n)) #6000x120
w.out     = rep(0, rrr)
mu.out    = array(0, dim=c(rrr, KK)) #6000x2
sigma.out = rep(0, rrr)
logpost   = rep(0, rrr)
...
for(s in 1:rrr){
  ...
  for(i in 1:n){
    ...
    cc[i] = sample(1:KK, 1, replace=TRUE, prob=v) #sample mixture components 1 or 2 from prob dists v[i]
  }

#beta, normal and invgamma posterior are derived algeraically and plugged in
#all conjugate posteriors from beta, normal, invgamma
  
  # Sample the weights
  w = rbeta(1, aa[1] + sum(cc==1), aa[2] + sum(cc==2)) #conjugate posterior for gamma(aa[1], aa[2]) prior after iterations
  
  # Sample the means
  for(k in 1:KK){
    nk    = sum(cc==k) #nb of elements drawn from components k
    xsumk = sum(x[cc==k]) 
