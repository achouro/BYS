...
plot(index_filt, interest[1:T], ylab = "Interest", 
     main = "Google Searches for Texas NLDM(1)",
...
FF <- (as.matrix(c(1,1)))
GG <- matrix(c(1,0,0,1),ncol=2,byrow=T)
VV <- as.matrix(1)
WW <- 1*diag(1)
m0 <- t(as.matrix(c(10,10)))
C0 <- 1*diag(1)
...
model2=dlmModPoly(order=2,dV=1,dW=1*rep(1,2),
                  m0=c(50,0),C0=1*diag(1))
# filtering and smoothing 
...
par(mfrow=c(1,1))
plot(index, interest,main = "Google Searches for Texas NLDM(2)", type='l',xlab="time",
     ylim=c(50,100))
